...
namespace Aquamancy.Services
{
    public class DiscordNotifierService : BackgroundService
    {
        ...
        private ILogger<DiscordNotifierService> _logger;

        public DiscordNotifierService(IServiceScopeFactory scopeFactory, IConfiguration configuration, ILogger<DiscordNotifierService> logger)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                catch (Exception ex)
                {
                    if (!_isError)
                    {
                         _logger.LogError("Erreur dans le service de notification Discord : " + ex.Message);
                        _isError = true;
                    }
                }
