...
namespace Aquamancy.Data
{
    public class ProbeRepository : IProbeRepository
    {
        ...
        public async Task<int> AddAsync(Probe probe)
        {
            ...
            await using var cmd = conn.CreateCommand();
            cmd.CommandText = "INSERT INTO probes (name, machine_name, color, min_temperature, max_temperature, send_frequency_in_seconds, tendency_span_hours, minimum_tendency_change, created_at, last_notified_at) VALUES (@name, @machine_name, @color, @min_temperature, @max_temperature, @send_frequency_in_seconds, @tendency_span_hours, @minimum_tendency_change, @created_at, @last_notified_at); SELECT LAST_INSERT_ID();";
        ...
        public async Task<Probe?> GetByIdAsync(int id)
        {
            ...
            await using var cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT id, name, machine_name, color, min_temperature, max_temperature, send_frequency_in_seconds, tendency_span_hours, minimum_tendency_change, created_at, last_notified_at FROM probes WHERE id = @id LIMIT 1";
        ...
        public async Task<IEnumerable<Probe>> GetAllAsync()
        {
            ...
            await using var cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT id, name, machine_name, color, min_temperature, max_temperature, send_frequency_in_seconds, tendency_span_hours, minimum_tendency_change, created_at, last_notified_at FROM probes ORDER BY id";
